...
CMyGraphManager::CMyGraphManager() :
...
last_pose(CPose3D())
{
	int numLevels = 1;// In this way, Hogman will only call CholOptimizer<PG>::optimize(), actually not online method!
	int nodeDistance = 2;
...
CMyGraphManager::~CMyGraphManager() {
	delete (optimizer_);
	for(map<int,CMyNode*>::iterator it=graph_.begin();it!=graph_.end();it++){
		delete it->second;
		it->second=NULL;
	}
}
...
//std::vector<int> CMyGraphManager::getPotentialEdgeTargets(const CMyNode* new_node, int max_targets){
//	int last_targets = 2; //always compare to the last n, spread evenly for the rest
//	int increment;
//	int counter = 0;
//
...
//	}
//
//	map <int, CMyNode*>::reverse_iterator graph_iter  = graph_.rbegin();
...
// in offline graph-based SLAM, we want to check every node
std::vector<int> CMyGraphManager::getPotentialEdgeTargets(const CMyNode* new_node, int max_targets){
	int last_targets = graph_.size()-1; //always compare to all the previous nodes
	std::vector<int> ids_to_link_to;
	...
	assert(last_targets==ids_to_link_to.size());
	return ids_to_link_to;
}
void CMyGraphManager::resetGraph(){
	int numLevels = 3;
	int nodeDistance = 2;
	marker_id =0;
	last_batch_update_=std::clock();
...
bool CMyGraphManager::addNode(CMyNode* new_node) {
	...
	if (graph_.size()==0){
		...
		optimizer_->addVertex(0, result, 1e9*Matrix6::eye(1.0)); //fix at origin
	...
	CMyNode* prev_frame = graph_.rbegin()->second;
	//	cout<<"IN MP "<<prev_frame->id_<<" "<<new_node->id_<<endl;

	/*cout<<"before match node id: "<<new_node->m_id<<endl;
	new_node->m_rootPose.displayROT();*/
	...
	for (int id_of_id = (int)vertices_to_comp.size()-1; id_of_id >=0;id_of_id--){ 
		CMyNode* abcd = graph_[vertices_to_comp[id_of_id]];
	...
	if (optimizer_->edges().size() > num_edges_before) { //Success
		...
		/*AISNavigation::PoseGraph3D::Vertex* v = optimizer_->vertex(new_node->m_id);
		...
		/*cout<<"1 after optimizer, id: "<<new_node->m_id<<endl;
		...
		/*CPose3D tmpPose;
		tmpPose.setxyz(latest_pose[0],latest_pose[1],latest_pose[2]);
		tmpPose.setrpy(latest_pose[3],latest_pose[4],latest_pose[5]);
		cout<<"1 after optimizer, id: "<<new_node->m_id<<endl;
		tmpPose.displayROT();*/
...
bool CMyGraphManager::isBigTrafo(const Transformation3& t){
...
bool CMyGraphManager::addEdgeToHogman(AIS::LoadedEdge3D edge, bool largeEdge) {
	std::clock_t starttime=std::clock();
	freshlyOptimized_ = false;
...
void CMyGraphManager::optimizeGraph(bool online){
...
void CMyGraphManager::reset(){
	reset_request_ = true;
}

void CMyGraphManager::initFirstNode(CPose3D initPose)
{
	last_pose = initPose;
}

void CMyGraphManager::deleteLastFrame(){
	if(graph_.size() <= 1) {
		reset_request_ = true;
		return;
	}
...
void CMyGraphManager::eraseNode(CMyNode* node){
	if(graph_.size() <= 1) {
...
bool CMyGraphManager::deleteOutRangeFrames(double x, double y, double z, double radius){
	bool removed = false;
	if(graph_.size() <= 1) {
		//      ROS_INFO("Resetting, as the only node is to be deleted");
		reset_request_ = true;
	...
	for(int i=0; i<vector_to_del.size(); i++){
		CMyNode* node_to_del=graph_[vector_to_del[i]];
		graph_.erase(node_to_del->m_id);
		delete node_to_del;
	}
	return removed;
}

void CMyGraphManager::setMaxDepth(float max_depth){
	Max_Depth = max_depth;
}
...
*/
void CMyGraphManager::testOptimizer(){
	// 1. add root node
