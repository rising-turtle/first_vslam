...
//  ROS_INFO_STREAM_COND_NAMED(( (std::clock()-starttime5) / (double)CLOCKS_PER_SEC) > global_min_time_reported, "timings", "pc2->pcl conversion runtime: " << ( std::clock() - starttime5 ) / (double)CLOCKS_PER_SEC );
  ...
  projectTo3D(feature_locations_2d_, feature_locations_3d_, *point_cloud); //takes less than 0.01 sec
  ...
  //pc_col.points.swap(std::vector<pcl::PointXYZRGB, Eigen::aligned_allocator<pcl::PointXYZRGB> >());
...
MatchingResult Node::matchNodePair(const Node* older_node){
  ...
  if (mr.all_matches.size() < min_matches){
	...
	bool out = getRelativeTransformationTo(older_node,&mr.all_matches, mr.ransac_trafo, mr.rmse, mr.inlier_matches);
